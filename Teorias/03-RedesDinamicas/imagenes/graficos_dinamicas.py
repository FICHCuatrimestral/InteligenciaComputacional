...
# ============================================================ FIGURA 14
def _tira_de_bits(ejes, x0, y0, valores, ancho=0.34, alto=0.34, resaltar=(),
                  color=COLOR_CLASE_A):
    """Dibuja un vector de +-1 como una fila de casillas llenas / vacias."""
    for columna, valor in enumerate(valores):
        distinto = columna in resaltar
        ejes.add_patch(Rectangle((x0 + columna * ancho, y0), ancho * 0.88, alto,
                                 facecolor=color if valor > 0 else "white",
                                 edgecolor=COLOR_CLASE_B if distinto else COLOR_GRILLA,
                                 linewidth=2.0 if distinto else 1.0, zorder=3))


def _mini_matriz(ejes, x0, y0, matriz, celda=0.22):
    """Dibuja una matriz de -1/0/+1 como un mosaico chiquito."""
    filas, columnas = matriz.shape
    for f in range(filas):
        for c in range(columnas):
            valor = matriz[f, c]
            if valor > 0:
                relleno, borde = COLOR_ACENTO, COLOR_ACENTO
            elif valor < 0:
                relleno, borde = COLOR_CLASE_B, COLOR_CLASE_B
            else:
                relleno, borde = "white", COLOR_GRILLA
            ejes.add_patch(Rectangle((x0 + c * celda, y0 - f * celda),
                                     celda * 0.9, celda * 0.9, facecolor=relleno,
                                     edgecolor=borde, linewidth=0.7, alpha=0.9,
                                     zorder=3))


def figura_dos_fases():
    """El mapa del algoritmo de Hopfield: escribir una vez, leer iterando."""
    x1 = np.array([1, 1, 1, 1, -1, -1, -1, -1])
    x2 = np.array([1, 1, -1, -1, 1, 1, -1, -1])
    W = np.outer(x1, x1) + np.outer(x2, x2)
    np.fill_diagonal(W, 0)
    sucio = x1.copy(); sucio[[1, 6]] *= -1

    figura, ejes = plt.subplots(figsize=(11.6, 5.4))
    ejes.set_xlim(0, 13.2); ejes.set_ylim(0, 7.4); limpiar(ejes)

    # --- carriles
    ejes.add_patch(FancyBboxPatch((0.15, 4.15), 12.9, 3.0,
                                  boxstyle="round,pad=0.06", linewidth=0,
                                  facecolor=COLOR_ACENTO, alpha=0.055, zorder=0))
    ejes.add_patch(FancyBboxPatch((0.15, 0.35), 12.9, 3.0,
                                  boxstyle="round,pad=0.06", linewidth=0,
                                  facecolor=COLOR_CLASE_A, alpha=0.055, zorder=0))
    ejes.text(0.42, 6.92, "FASE 1 · ALMACENAR   —   una sola pasada, sin iterar",
              fontsize=9.6, color=COLOR_ACENTO, weight="bold", va="top")
    ejes.text(0.42, 3.12, "FASE 2 · RECUPERAR   —   iterativa, una neurona por vez",
              fontsize=9.6, color=COLOR_CLASE_A, weight="bold", va="top")

    # --- fase 1
    ejes.text(1.55, 6.28, "memorias fundamentales", fontsize=8.6, ha="center",
              color=TINTA_SECUNDARIA, style="italic")
    for fila, (patron, etiqueta) in enumerate([(x1, "$\\mathbf{x}^*_1$"),
                                               (x2, "$\\mathbf{x}^*_2$")]):
        _tira_de_bits(ejes, 0.85, 5.55 - fila * 0.5, patron, color=COLOR_ACENTO)
        ejes.text(0.68, 5.72 - fila * 0.5, etiqueta, ha="right", va="center", fontsize=9)
    flecha(ejes, (3.72, 5.47), (4.02, 5.47), COLOR_ACENTO, 1.6)

    caja(ejes, (5.35, 5.55), 2.5, 1.35,
         "regla de Hebb\n$w_{ji}=\\frac{1}{N}\\sum_k x^*_{kj}x^*_{ki}$\n$w_{ii}=0$",
         COLOR_ACENTO, tamano=9)
    flecha(ejes, (6.65, 5.55), (7.75, 5.55), COLOR_ACENTO, 1.6)

    _mini_matriz(ejes, 8.05, 6.42, W)
    ejes.text(8.93, 4.42, "$\\mathbf{W}$: $N\\times N$, simétrica, diagonal nula",
              ha="center", va="top", fontsize=8.6, color=TINTA_SECUNDARIA)
    ejes.text(11.6, 5.55, "no hay épocas,\nno hay error,\nno hay $\\mu$:\n**termina acá**".replace("**", ""),
              ha="center", va="center", fontsize=8.8, color=COLOR_ACENTO, style="italic")

    # --- puente entre fases
    flecha(ejes, (8.93, 4.16), (8.93, 2.62), TINTA_SECUNDARIA, 1.2, guiones=(0, (4, 3)))
    ejes.text(9.12, 3.4, "la misma $\\mathbf{W}$, ya fija:\nde acá en más no se toca",
              fontsize=8.4, va="center", color=TINTA_SECUNDARIA, style="italic")

    # --- fase 2
    ejes.text(1.55, 2.42, "patrón sucio (entrada)", fontsize=8.6, ha="center",
              color=TINTA_SECUNDARIA, style="italic")
    _tira_de_bits(ejes, 0.85, 1.72, sucio, resaltar=(1, 6))
    ejes.text(0.68, 1.89, "$\\mathbf{x}$", ha="right", va="center", fontsize=9)
    ejes.text(1.55, 1.45, "dos bits dados vuelta", fontsize=8, ha="center",
              va="top", color=COLOR_CLASE_B)
    flecha(ejes, (3.72, 1.89), (4.02, 1.89), COLOR_CLASE_A, 1.6)

    caja(ejes, (5.35, 1.89), 2.5, 1.35,
         "$\\mathbf{y}(0)=\\mathbf{x}$\nsorteo $j^*$ y recalculo\n"
         "$y_{j^*}=\\mathrm{sgn}(\\sum_i w_{j^*i}y_i)$",
         COLOR_CLASE_A, tamano=8.6)
    # bucle
    bucle = FancyArrowPatch((6.15, 1.12), (4.55, 1.12), arrowstyle="-|>",
                            mutation_scale=12, color=COLOR_CLASE_A, linewidth=1.3,
                            connectionstyle="arc3,rad=-0.45", zorder=6,
                            shrinkA=0, shrinkB=0)
    ejes.add_patch(bucle)
    ejes.text(5.35, 0.52, "mientras alguna cambie", ha="center", fontsize=8.2,
              color=COLOR_CLASE_A, style="italic")

    flecha(ejes, (6.65, 1.89), (7.75, 1.89), COLOR_CLASE_A, 1.6)
    _tira_de_bits(ejes, 7.95, 1.72, x1, color=COLOR_ACENTO)
    ejes.text(10.85, 1.89, "$=\\mathbf{y}(M)$", ha="left", va="center", fontsize=9.5)
    ejes.text(10.85, 1.42, "una pasada completa\nsin un solo cambio", ha="left", va="top",
              fontsize=8.4, color=TINTA_SECUNDARIA, style="italic")

    guardar(figura, "14-dos-fases.png")


# ============================================================ FIGURA 15
def figura_construccion_de_w():
    """De los dos patrones a la matriz: producto externo, suma, diagonal nula."""
    x1 = np.array([1, 1, 1, 1, -1, -1, -1, -1])
    x2 = np.array([1, 1, -1, -1, 1, 1, -1, -1])
    A, B = np.outer(x1, x1), np.outer(x2, x2)
    S = A + B
    W = S.copy(); np.fill_diagonal(W, 0)

    figura, paneles = plt.subplots(1, 4, figsize=(12.4, 3.9))
    datos = [
        (A, "$\\mathbf{x}^*_1(\\mathbf{x}^*_1)^{\\mathsf{T}}$",
         "cada entrada: $+1$ si las dos\nposiciones tienen el mismo signo"),
        (B, "$\\mathbf{x}^*_2(\\mathbf{x}^*_2)^{\\mathsf{T}}$",
         "lo mismo para el segundo patrón"),
        (S, "la suma",
         "coinciden $\\rightarrow \\pm2$,\ndifieren $\\rightarrow 0$"),
        (W, "$8\\,\\mathbf{W}$", "y se fuerza la diagonal a cero"),
    ]
    for ejes, (matriz, titulo, pie) in zip(paneles, datos):
        ejes.imshow(matriz, cmap="RdBu_r", vmin=-2.6, vmax=2.6)
        for f in range(8):
            for c in range(8):
                v = matriz[f, c]
                ejes.text(c, f, f"{v:+d}".replace("+0", "0"), ha="center", va="center",
                          fontsize=6.4,
                          color=TINTA_PRIMARIA if abs(v) < 2 else "white")
        ejes.set_xticks(np.arange(-0.5, 8, 1), minor=True)
        ejes.set_yticks(np.arange(-0.5, 8, 1), minor=True)
        ejes.grid(which="minor", color=SUPERFICIE, linewidth=1.2)
        ejes.tick_params(which="both", length=0, labelbottom=False, labelleft=False)
        ejes.set_title(titulo, fontsize=10.5)
        ejes.text(3.5, 8.9, pie, ha="center", va="top", fontsize=8.2,
                  color=TINTA_SECUNDARIA, style="italic")

    # las tres entradas que se calculan a mano en el texto
    for columna, fila, etiqueta, color, desplazamiento in [
            (1, 0, "$w_{12}$", COLOR_ACENTO, -16),
            (6, 0, "$w_{17}$", COLOR_CLASE_B, 0),
            (2, 0, "$w_{13}$", TINTA_SECUNDARIA, 18)]:
        paneles[3].add_patch(Rectangle((columna - 0.5, fila - 0.5), 1, 1,
                                       facecolor="none", edgecolor=color,
                                       linewidth=2.2, zorder=5))
        paneles[3].annotate(etiqueta, (columna, fila), xytext=(desplazamiento, 36),
                            textcoords="offset points", ha="center", fontsize=8.4,
                            color=color,
                            arrowprops=dict(arrowstyle="->", color=color, linewidth=1.0))
    figura.text(0.5, -0.03,
                "Las tres entradas marcadas son los tres casos de Hebb: "
                "siempre iguales, siempre opuestas, y sin relación.",
                ha="center", fontsize=8.6, color=TINTA_SECUNDARIA, style="italic")
    guardar(figura, "15-construccion-de-w.png")


# ============================================================ FIGURA 16
def figura_una_iteracion():
    """Que pasa adentro de UNA iteracion: la votacion ponderada del resto."""
    estado = np.array([1, -1, 1, 1, -1, -1, -1, -1], dtype=float)
    pesos_fila_2 = np.array([2, 0, 0, 0, 0, 0, -2, -2]) / 8.0
    aportes = pesos_fila_2 * estado
    total = aportes.sum()

    figura, paneles = plt.subplots(1, 3, figsize=(12.6, 3.8),
                                   gridspec_kw={"width_ratios": [1.0, 1.35, 0.85]})
    izq, medio, der = paneles

    # --- panel 1: el estado y quien se actualiza
    izq.set_xlim(-1.4, 8.6); izq.set_ylim(-2.2, 2.9); limpiar(izq)
    for columna, valor in enumerate(estado):
        elegida = columna == 1
        izq.add_patch(Rectangle((columna, 0.35), 0.86, 0.86,
                                facecolor=COLOR_CLASE_A if valor > 0 else "white",
                                edgecolor=COLOR_CLASE_B if elegida else COLOR_GRILLA,
                                linewidth=2.6 if elegida else 1.0, zorder=3))
        izq.text(columna + 0.43, 0.08, f"${columna+1}$", ha="center", va="top",
                 fontsize=8, color=COLOR_CLASE_B if elegida else TINTA_SECUNDARIA)
    izq.text(-0.25, 0.78, "$\\mathbf{y}(n-1)$", ha="right", va="center", fontsize=9)
    flecha(izq, (1.43, 2.05), (1.43, 1.32), COLOR_CLASE_B, 1.4)
    izq.text(1.43, 2.22, "sale sorteada: $j^*=2$", ha="center", va="bottom",
             fontsize=9, color=COLOR_CLASE_B)
    izq.text(3.6, -1.15, "las otras siete no se tocan\nen esta iteración",
             ha="center", va="center", fontsize=8.8, color=TINTA_SECUNDARIA,
             style="italic")
    izq.set_title("1. Se sortea una neurona", fontsize=10, color=TINTA_SECUNDARIA)

    # --- panel 2: la votacion ponderada, con la suma como ultima barra
    limpiar(medio, COLOR_GRILLA)
    indices = np.arange(1, 9)
    colores = [COLOR_ACENTO if a > 0 else (COLOR_CLASE_B if a < 0 else COLOR_GRILLA)
               for a in aportes]
    medio.bar(indices, aportes, color=colores, width=0.6, zorder=3)
    medio.bar([9.6], [total], color=COLOR_CLASE_A, width=0.6, zorder=3)
    for i, a in enumerate(aportes, start=1):
        if a == 0:
            medio.text(i, 0.015, "$0$", ha="center", va="bottom", fontsize=8,
                       color=TINTA_SECUNDARIA)
        else:
            medio.text(i, a + 0.015, "$+0{,}25$", ha="center", va="bottom",
                       fontsize=7.6, color=TINTA_PRIMARIA)
    medio.text(9.6, total + 0.02, "$v_2 = +0{,}75$", ha="center", va="bottom",
               fontsize=9.4, color=COLOR_CLASE_A)
    medio.axhline(0, color=TINTA_SECUNDARIA, linewidth=1.0, zorder=4)
    medio.axvline(8.8, color=COLOR_GRILLA, linewidth=1.2, linestyle="--", zorder=1)
    medio.grid(axis="y", color=COLOR_GRILLA, linewidth=0.8, zorder=0)
    medio.set_xlim(0.2, 10.3); medio.set_ylim(-0.06, 0.98)
    medio.set_xticks(list(indices) + [9.6])
    medio.set_xticklabels([str(i) for i in indices] + ["suma"], fontsize=8)
    medio.set_yticks([0, 0.25, 0.5, 0.75])
    medio.set_yticklabels(["$0$", "$0{,}25$", "$0{,}50$", "$0{,}75$"], fontsize=8)
    medio.set_xlabel("neurona $i$ que aporta", fontsize=9)
    medio.set_ylabel("$w_{2i}\\,y_i$", fontsize=9.5)
    medio.text(3.5, 0.62,
               "las neuronas 3 a 6 tienen $w_{2i}=0$:\nno opinan sobre la 2",
               ha="center", fontsize=8.4, color=TINTA_SECUNDARIA, style="italic")
    medio.set_title("2. Cada una vota, pesada por $w_{2i}$", fontsize=10,
                    color=TINTA_SECUNDARIA)

    # --- panel 3: el sgn
    limpiar(der, COLOR_GRILLA)
    v = np.linspace(-1, 1, 400)
    der.plot(v[v < 0], np.full((v < 0).sum(), -1), color=COLOR_CLASE_A, linewidth=2.0)
    der.plot(v[v > 0], np.full((v > 0).sum(), 1), color=COLOR_CLASE_A, linewidth=2.0)
    der.axvline(total, color=COLOR_ACENTO, linestyle=":", linewidth=1.3)
    der.scatter([total], [1], s=80, color=COLOR_ACENTO, zorder=5, linewidth=0)
    der.annotate("$y_2 \\leftarrow +1$\n(era $-1$: cambió)", (total, 1),
                 xytext=(-58, -52), textcoords="offset points", ha="center",
                 fontsize=9, color=COLOR_ACENTO,
                 arrowprops=dict(arrowstyle="->", color=COLOR_ACENTO, linewidth=1.2))
    der.grid(color=COLOR_GRILLA, linewidth=0.8, zorder=0)
    der.set_xlim(-1, 1); der.set_ylim(-1.7, 1.7)
    der.set_xticks([-1, 0, total, 1])
    der.set_xticklabels(["$-1$", "$0$", "$0{,}75$", "$1$"], fontsize=8)
    der.set_yticks([-1, 1]); der.set_yticklabels(["$-1$", "$+1$"], fontsize=8)
    der.set_xlabel("$v_2$", fontsize=9.5)
    der.set_ylabel("$\\mathrm{sgn}(v_2)$", fontsize=9.5)
    der.set_title("3. El signo decide el nuevo valor", fontsize=10,
                  color=TINTA_SECUNDARIA)

    figura.text(0.5, -0.06,
                "Una iteración completa. Volver al panel 1 y repetir; "
                "cuando se recorren las ocho sin un solo cambio, terminó.",
                ha="center", fontsize=8.8, color=TINTA_SECUNDARIA, style="italic")
    guardar(figura, "16-una-iteracion.png")
...
if __name__ == "__main__":
    ...
    figura_elman_jordan()
    figura_energia_del_ejemplo()
    figura_dos_fases()
    figura_construccion_de_w()
    figura_una_iteracion()
